...
function initMathCanvas() {
  ...
  setTimeout(() => {
    const P = {
      ...
      muted: "#857f72",
      amber: "#8a6818",
      amberF: "rgba(138,104,24,.11)",
      amberS: "rgba(138,104,24,.6)",
      teal: "#1a6e5c",
      tealF: "rgba(26,110,92,.11)",
      tealS: "rgba(26,110,92,.6)",
      red: "#8a2e20",
      redF: "rgba(138,46,32,.1)",
      redS: "rgba(138,46,32,.55)",
      grid: "rgba(0,0,0,.055)",
      axis: "rgba(28,26,22,.45)",
      curve: "#1c1a16",
    }
    ...
    function axes(ctx, T, M, W, H, xMn, xMx, yMn, yMx, xs, ys) {
      ...
      ctx.beginPath()
      ctx.moveTo(M.l, axY)
      ctx.lineTo(W - M.r + 8, axY)
      ctx.stroke()
      ctx.beginPath()
      ctx.moveTo(axX, M.t - 8)
      ctx.lineTo(axX, H - M.b)
      ctx.stroke()
    ...
    function bands(ctx, T, M, W, H, c, L, eps, delta, yMn, yMx, xMn, xMx) {
      ...
      ctx.fillStyle = P.amberF
      ctx.fillRect(M.l, yT, W - M.r - M.l, yB - yT)
      ctx.strokeStyle = P.amberS
      ctx.lineWidth = 1.2
      ctx.setLineDash([5, 3])
      ctx.beginPath()
      ctx.moveTo(M.l, yT)
      ...
      ctx.stroke()
      ctx.setLineDash([])
      ctx.fillStyle = P.tealF
      ctx.fillRect(xL, M.t, xR2 - xL, H - M.t - M.b)
      ctx.strokeStyle = P.tealS
      ctx.lineWidth = 1.2
      ctx.setLineDash([5, 3])
      ctx.beginPath()
      ctx.moveTo(xL, M.t)
      ...
      ctx.beginPath()
      ctx.moveTo(xR2, M.t)
      ctx.lineTo(xR2, H - M.b)
      ctx.stroke()
      ctx.setLineDash([])
      ctx.fillStyle = P.amber
      ctx.font = "italic 11px serif"
      ctx.textAlign = "left"
      ctx.fillText("L+ε", M.l + 3, yT - 3)
      ctx.fillText("L−ε", M.l + 3, yB + 11)
      ctx.fillStyle = P.teal
      ctx.textAlign = "center"
      ctx.fillText("c−δ", xL, H - M.b + 13)
      ctx.fillText("c+δ", xR2, H - M.b + 13)
    }
    function curve(ctx, T, fn, x0, x1, step, skipNear) {
      ...
      for (let x = x0; x <= x1 + step * 0.5; x += step) {
        ...
        if (skipNear !== undefined && Math.abs(xc - skipNear) < step * 1.5) {
          go = false
          continue
        }
        const y = fn(xc)
        ...
        const p = T(xc, y)
        go ? ctx.lineTo(p.x, p.y) : ctx.moveTo(p.x, p.y)
        go = true
      }
      ctx.stroke()
    }
    function dot(ctx, T, x, y, fill, r, col) {
      ...
      if (fill) {
        ctx.fillStyle = col || P.amber
        ctx.fill()
      } else {
        ctx.strokeStyle = col || P.amber
        ctx.lineWidth = 2
        ctx.stroke()
    ...
    function guide(ctx, T, x, y, M, H) {
      ctx.setLineDash([4, 3])
      ctx.strokeStyle = "rgba(138,104,24,.3)"
      ctx.lineWidth = 1
      const p = T(x, y)
      ctx.beginPath()
      ctx.moveTo(p.x, p.y)
    ...
    document.querySelectorAll('[id="nlC"]').forEach((cv) => {
      ...
      function lp(x) {
        return M.l + ((x - xMn) / (xMx - xMn)) * (W - M.l - M.r)
      }
      const y = H / 2 + 6
      ctx.fillStyle = P.bg
      ...
      ctx.beginPath()
      ctx.moveTo(M.l - 12, y)
      ctx.lineTo(W - M.r + 12, y)
      ctx.stroke()
      for (let x = 0; x <= 4; x += 0.5) {
        const px = lp(x),
          ii = Number.isInteger(x)
        ctx.strokeStyle = ii ? P.axis : "rgba(28,26,22,.25)"
        ctx.lineWidth = ii ? 1 : 0.5
        ctx.beginPath()
        ctx.moveTo(px, y - (ii ? 7 : 4))
        ctx.lineTo(px, y + (ii ? 7 : 4))
        ctx.stroke()
        if (ii) {
          ctx.fillStyle = P.muted
          ctx.font = "11px monospace"
          ctx.textAlign = "center"
          ctx.fillText(x, px, y + 17)
        }
      }
      ctx.fillStyle = P.teal
      ctx.font = "italic 12px serif"
      ctx.textAlign = "center"
      ctx.fillText("c−δ = 1.2", lx, y - 21)
      ctx.fillText("c+δ = 2.8", rx, y - 21)
      const cx_ = lp(c)
      ctx.beginPath()
      ctx.arc(cx_, y, 7, 0, Math.PI * 2)
      ...
      ctx.fillStyle = "rgba(138,104,24,.55)"
    ...
    document.querySelectorAll('[id="ex1C"]').forEach((cv) => {
      ...
      function draw(eps) {
        ...
        ctx.font = "italic 12px serif"
        ctx.textAlign = "right"
        ctx.fillText("L=4", M.l - 3, pCL.y + 4)
        ctx.textAlign = "center"
        ctx.fillText("c=2", pCL.x, H - M.b + 13)
        ctx.fillStyle = P.muted
        ctx.font = "11px sans-serif"
        ctx.textAlign = "right"
        ctx.fillText("f(x) = x²", W - M.r - 4, M.t + 13)
      }
      function upd() {
        if (!sl) return
        const e = +sl.value,
    ...
    document.querySelectorAll('[id="ex2C"]').forEach((cv) => {
      ...
      function draw(eps) {
        ...
        curve(ctx, T, (x) => x + 2, xMn, xMx, 0.015, C)
        dot(ctx, T, C, L, false)
        guide(ctx, T, C, L, M, H)
        const pCL = T(C, L)
        ctx.fillStyle = P.amber
        ...
        ctx.fillStyle = P.red
        ctx.font = "10px sans-serif"
        ctx.fillText("ثقب — غير معرّف", pCL.x, pCL.y - 15)
        ctx.fillStyle = P.muted
        ctx.font = "11px sans-serif"
        ctx.textAlign = "right"
        ctx.fillText("f(x) = (x²−4)/(x−2)", W - M.r - 2, M.t + 13)
      }
      function upd() {
        if (!sl) return
        const e = +sl.value
    ...
    document.querySelectorAll('[id="ex3C"]').forEach((cv) => {
      ...
      function cmpDelta(eps) {
        ...
        for (let i = 0; i < 50; i++) {
          const m = (lo + hi) / 2
          Math.abs(Math.sin(m) / m - 1) < eps ? (lo = m) : (hi = m)
        }
        return (lo + hi) / 2
      }
      function draw(eps) {
        ...
        axes(ctx, T, M, W, H, xMn, xMx, yMn, yMx, 1, 0.5)
        curve(ctx, T, (x) => Math.sin(x) / x, xMn, xMx, 0.015, C)
        dot(ctx, T, C, L, false)
        const pCL = T(C, L)
        ctx.setLineDash([4, 3])
        ctx.strokeStyle = "rgba(138,104,24,.3)"
        ...
        ctx.lineTo(M.l, pCL.y)
        ctx.stroke()
        ctx.setLineDash([])
        ctx.fillStyle = P.amber
        ctx.font = "italic 12px serif"
        ctx.textAlign = "right"
        ctx.fillText("L=1", M.l - 3, pCL.y + 4)
        ctx.textAlign = "center"
        ctx.fillText("c=0", pCL.x, H - M.b + 13)
        ctx.fillStyle = P.muted
        ctx.font = "11px sans-serif"
        ctx.textAlign = "right"
        ctx.fillText("f(x) = sin(x)/x", W - M.r - 2, M.t + 13)
    ...
    document.querySelectorAll('[id="ex4C"]').forEach((cv) => {
      ...
      function draw(L) {
        ...
        ctx.fillStyle = P.bg
        ctx.fillRect(0, 0, W, H)
        const yT = T(0, Math.min(L + EPS, yMx)).y,
          yB = T(0, Math.max(L - EPS, yMn)).y
        ctx.fillStyle = P.amberF
        ctx.fillRect(M.l, yT, W - M.r - M.l, yB - yT)
        ctx.strokeStyle = P.amberS
        ctx.lineWidth = 1.2
        ...
        ctx.stroke()
        ctx.setLineDash([])
        axes(ctx, T, M, W, H, xMn, xMx, yMn, yMx, 0.5, 0.5)
        ctx.strokeStyle = P.curve
        ctx.lineWidth = 2.5
        const pa = T(0.01, 1),
          pb = T(xMx, 1)
        ...
        const plusIn = Math.abs(1 - L) < EPS,
          minusIn = Math.abs(-1 - L) < EPS
        if (!plusIn) {
          ...
          ctx.font = "10px sans-serif"
          ctx.textAlign = "center"
          ctx.fillText("خارج", pp.x, pp.y - 15)
        }
        if (!minusIn) {
          ctx.strokeStyle = P.red
          ctx.lineWidth = 2
          const pm = T(-0.8, -1)
          ctx.beginPath()
          ctx.arc(pm.x, pm.y, 11, 0, Math.PI * 2)
          ctx.stroke()
          ctx.fillStyle = P.red
          ctx.font = "10px sans-serif"
          ctx.textAlign = "center"
          ctx.fillText("خارج", pm.x, pm.y + 18)
        }
